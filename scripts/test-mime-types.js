...
async function testMimeTypes() {
  ...
  for (const asset of assetsToTest) {
    try {
      ...
      const typeMatches = contentType && contentType.includes(asset.expectedType);
  ...
  if (failCount > 0) {
    console.log(chalk ? chalk.yellow('Warning: Some assets failed the MIME type test') : 'Warning: Some assets failed the MIME type test');
    console.log('Ensure your middleware and server configurations are correctly set up.');
  } else {
    console.log(chalk ? chalk.green('Success: All assets have correct MIME types') : 'Success: All assets have correct MIME types');
  }
}
