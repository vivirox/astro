\doxysection{node\+\_\+modules/fflate Directory Reference}
\hypertarget{dir_659b8a9d70a61e2b6195c42f9fd52df8}{}\label{dir_659b8a9d70a61e2b6195c42f9fd52df8}\index{node\_modules/fflate Directory Reference@{node\_modules/fflate Directory Reference}}


\doxysubsection{Detailed Description}
High performance (de)compression in an 8kB package\hypertarget{README.md_autotoc_md15427}{}\doxysubsection{\texorpdfstring{Why fflate?}{Why fflate?}}\label{README.md_autotoc_md15427}
...
\DoxyCodeLine{with\ only\ the\ features\ you\ need.\ This\ build\ is\ about\ 31kB,\ or\ 11.5kB\ gzipped.}
\DoxyCodeLine{-\/-\/>}
\DoxyCodeLine{<script\ src="{}https://unpkg.com/fflate@0.8.2"{}></script>}
\DoxyCodeLine{<script\ src="{}https://cdn.jsdelivr.net/npm/fflate@0.8.2/umd/index.js"{}></script>}
...
\DoxyCodeLine{\ \ import\ *\ as\ fflate\ from\ 'https://cdn.skypack.dev/fflate@0.8.2?min';}
\DoxyCodeLine{</script>}
...
\DoxyCodeLine{}
\DoxyCodeLine{//\ @deno-\/types="{}https://cdn.skypack.dev/fflate@0.8.2/lib/index.d.ts"{}}
\DoxyCodeLine{import\ *\ as\ fflate\ from\ 'https://cdn.skypack.dev/fflate@0.8.2?min';}
...
Note that there is a significant initial overhead to using workers of about 50ms for each asynchronous function. For instance, if you call {\ttfamily unzip} ten times, the overhead only applies for the first call, but if you call {\ttfamily unzip} and {\ttfamily zlib}, they will each cause the 50ms delay. For small (under about 50kB) payloads, the asynchronous APIs will be much slower. However, if you\textquotesingle{}re compressing larger files/multiple files at once, or if the synchronous API causes the main thread to hang for too long, the callback APIs are an order of magnitude better. 
\begin{DoxyCode}{0}
\DoxyCodeLine{import\ \{}
...
See the \href{https://github.com/101arrowz/fflate/blob/master/docs/README.md}{\texttt{ documentation}} for more detailed information about the API.\hypertarget{README.md_autotoc_md15430}{}\doxysubsection{\texorpdfstring{Bundle size estimates}{Bundle size estimates}}\label{README.md_autotoc_md15430}
The bundle size measurements for {\ttfamily fflate} on sites like Bundlephobia include every feature of the library and should be seen as an upper bound. As long as you are using tree shaking or dead code elimination, this table should give you a general idea of {\ttfamily fflate}\textquotesingle{}s bundle size for the features you need.

The maximum bundle size that is possible with {\ttfamily fflate} is about 31kB (11.\+5kB gzipped) if you use every single feature, but feature parity with {\ttfamily pako} is only around 10kB (as opposed to 45kB from {\ttfamily pako}). If your bundle size increases dramatically after adding {\ttfamily fflate}, please \href{https://github.com/101arrowz/fflate/issues/new}{\texttt{ create an issue}}.
...
Compression   &5kB   &{\ttfamily UZIP.\+js}, 2.\+84x larger    \\\cline{1-3}
Async decompression   &4kB (1kB + raw decompression)   &N/A    \\\cline{1-3}
Async compression   &6kB (1kB + raw compression)   &N/A    \\\cline{1-3}
ZIP decompression   &5kB (2kB + raw decompression)   &{\ttfamily UZIP.\+js}, 2.\+84x larger    \\\cline{1-3}
ZIP compression   &7kB (2kB + raw compression)   &{\ttfamily UZIP.\+js}, 2.\+03x larger    \\\cline{1-3}
GZIP/\+Zlib decompression   &4kB (1kB + raw decompression)   &{\ttfamily pako}, 11.\+4x larger    \\\cline{1-3}
GZIP/\+Zlib compression   &5kB (1kB + raw compression)   &{\ttfamily pako}, 9.\+12x larger    \\\cline{1-3}
Streaming decompression   &4kB (1kB + raw decompression)   &{\ttfamily pako}, 11.\+4x larger    \\\cline{1-3}
Streaming compression   &5kB (1kB + raw compression)   &{\ttfamily pako}, 9.\+12x larger   \\\cline{1-3}
\end{longtabu}
\hypertarget{README.md_autotoc_md15431}{}\doxysubsection{\texorpdfstring{What makes {\ttfamily fflate} so fast?}{What makes {\ttfamily fflate} so fast?}}\label{README.md_autotoc_md15431}
...
Before you decide that {\ttfamily fflate} is the end-\/all compression library, you should note that Java\+Script simply cannot rival the performance of a native program. If you\textquotesingle{}re only using Node.\+js, it\textquotesingle{}s probably better to use the \href{https://nodejs.org/api/zlib.html}{\texttt{ native Zlib bindings}}, which tend to offer the best performance. Though note that even against Zlib, {\ttfamily fflate} is only around 30\% slower in decompression and 10\% slower in compression, and can still achieve better compression ratios!\hypertarget{README.md_autotoc_md15432}{}\doxysubsection{\texorpdfstring{What about {\ttfamily Compression\+Stream}?}{What about {\ttfamily Compression\+Stream}?}}\label{README.md_autotoc_md15432}
Like {\ttfamily fflate}, the \href{https://developer.mozilla.org/en-US/docs/Web/API/Compression_Streams_API}{\texttt{ Compression Streams API}} provides DEFLATE, GZIP, and Zlib compression and decompression support. It\textquotesingle{}s a good option if you\textquotesingle{}d like to compress or decompress data without installing any third-\/party libraries, and it wraps native Zlib bindings to achieve better performance than what most Java\+Script programs can achieve.

However, browsers do not offer any native non-\/streaming compression API, and {\ttfamily Compression\+Stream} has surprisingly poor performance on data already loaded into memory; {\ttfamily fflate} tends to be faster even for files that are dozens of megabytes large. Similarly, {\ttfamily fflate} is much faster for files under a megabyte because it avoids marshalling overheads. Even when streaming hundreds of megabytes of data, the native API usually performs between 30\% faster and 10\% slower than {\ttfamily fflate}. And Compression Streams have many other disadvantages -\/ no ability to control compression level, poor support for older browsers, no ZIP support, etc.

If you\textquotesingle{}d still prefer to depend upon a native browser API but want to support older browsers, you can use an {\ttfamily fflate}-\/based \href{https://github.com/101arrowz/compression-streams-polyfill}{\texttt{ Compression Streams ponyfill}}.\hypertarget{README.md_autotoc_md15433}{}\doxysubsection{\texorpdfstring{Browser support}{Browser support}}\label{README.md_autotoc_md15433}
...
Other than that, {\ttfamily fflate} is completely ES3, meaning you probably won\textquotesingle{}t even need a bundler to use it.\hypertarget{README.md_autotoc_md15434}{}\doxysubsection{\texorpdfstring{Testing}{Testing}}\label{README.md_autotoc_md15434}
You can validate the performance of {\ttfamily fflate} with {\ttfamily npm test}. It validates that the module is working as expected, ensures the outputs are no more than 5\% larger than competitors at max compression, and outputs performance metrics to {\ttfamily test/results}.

Note that the time it takes for the CLI to show the completion of each test is not representative of the time each package took, so please check the JSON output if you want accurate measurements.\hypertarget{README.md_autotoc_md15435}{}\doxysubsection{\texorpdfstring{License}{License}}\label{README.md_autotoc_md15435}
