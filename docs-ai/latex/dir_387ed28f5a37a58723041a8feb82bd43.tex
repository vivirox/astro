\doxysection{node\+\_\+modules/regex Directory Reference}
\hypertarget{dir_387ed28f5a37a58723041a8feb82bd43}{}\label{dir_387ed28f5a37a58723041a8feb82bd43}\index{node\_modules/regex Directory Reference@{node\_modules/regex Directory Reference}}
...
\hypertarget{README.md_autotoc_md28537}{}\doxysubsubsection{\texorpdfstring{💎 Features}{💎 Features}}\label{README.md_autotoc_md28537}
...
\end{DoxyItemize}\hypertarget{README.md_autotoc_md28538}{}\doxysubsubsection{\texorpdfstring{🕹️ Install and use}{🕹️ Install and use}}\label{README.md_autotoc_md28538}
...
\DoxyCodeLine{regex`\(\backslash\)w`.test(str);\ //\ →\ true}
\DoxyCodeLine{str.match(regex('g')`\(\backslash\)w`);\ //\ →\ ['a',\ 'b',\ 'c']}
...
\end{DoxyCode}
 \end{adjustwidth}
\hypertarget{README.md_autotoc_md28539}{}\doxysubsubsection{\texorpdfstring{🪧 Examples}{🪧 Examples}}\label{README.md_autotoc_md28539}
...
\DoxyCodeLine{//\ →\ /\string^(?<first>.)(.)\(\backslash\)2(.)\(\backslash\)3\$/v}

\end{DoxyCode}


See also this example of using a subroutine definition group to \href{https://x.com/slevithan/status/1828112006353953055}{\texttt{ refactor an IP address regex for readability}}.\hypertarget{README.md_autotoc_md28540}{}\doxysubsubsection{\texorpdfstring{❓ Context}{❓ Context}}\label{README.md_autotoc_md28540}
...
{\bfseries{👉 {\bfseries{Learn more with examples}}}}\begin{adjustwidth}{1em}{0em}
...
For a more useful example, consider how this can affect lazy (non-\/greedy) quantifiers. Let\textquotesingle{}s say you want to match {\ttfamily \texorpdfstring{$<$}{<}b\texorpdfstring{$>$}{>}…\texorpdfstring{$<$}{<}/b\texorpdfstring{$>$}{>}} tags that are followed by {\ttfamily !}. You might try this\+:
...
\end{DoxyNote}
\hypertarget{README.md_autotoc_md28543}{}\doxysubsubsubsection{\texorpdfstring{Possessive quantifiers}{Possessive quantifiers}}\label{README.md_autotoc_md28543}
...
\end{DoxyNote}
\hypertarget{README.md_autotoc_md28544}{}\doxysubsubsubsection{\texorpdfstring{Subroutines}{Subroutines}}\label{README.md_autotoc_md28544}
...
{\bfseries{👉 {\bfseries{Show how to define subpatterns for use by reference only}}}}\begin{adjustwidth}{1em}{0em}
...
Above, the {\ttfamily \{0\}} quantifier at the end of the {\ttfamily (?\texorpdfstring{$<$}{<}byte\texorpdfstring{$>$}{>}…)} group allows {\itshape defining} the group without {\itshape matching} it at that position. The subpattern within it can then be used by reference elsewhere within the pattern.
...
\end{DoxyNote}
\hypertarget{README.md_autotoc_md28545}{}\doxysubsubsubsection{\texorpdfstring{Subroutine definition groups}{Subroutine definition groups}}\label{README.md_autotoc_md28545}
The syntax {\ttfamily (?(DEFINE)…)} can be used at the end of a regex to define subpatterns for use by reference only. When combined with subroutines, this enables writing regexes in a grammatical way that can significantly improve readability and maintainability.
...
\DoxyCodeLine{const\ match\ =\ record.match(re);}
...
{\ttfamily Reg\+Exp} instances interpolated into the pattern preserve their own flags locally (see \texorpdfstring{$\ast$}{*}\+Interpolating regexes\texorpdfstring{$\ast$}{*}).\hypertarget{README.md_autotoc_md28548}{}\doxysubsubsubsection{\texorpdfstring{Implicit flags}{Implicit flags}}\label{README.md_autotoc_md28548}
...
\hypertarget{README.md_autotoc_md28549}{}\doxysubsubsubsection{\texorpdfstring{Flag {\ttfamily v}}{Flag {\ttfamily v}}}\label{README.md_autotoc_md28549}
...
\hypertarget{README.md_autotoc_md28550}{}\doxysubsubsubsection{\texorpdfstring{Flag {\ttfamily x}}{Flag {\ttfamily x}}}\label{README.md_autotoc_md28550}
...
\item Whitespace is not insignificant within most enclosed tokens like {\ttfamily \textbackslash{}p\{…\}} and {\ttfamily \textbackslash{}u\{…\}}. The exception is {\ttfamily \mbox{[}\textbackslash{}q\{…\}\mbox{]}}.
...
\end{DoxyItemize}\end{adjustwidth}
\hypertarget{README.md_autotoc_md28551}{}\doxysubsubsubsection{\texorpdfstring{Flag {\ttfamily n}}{Flag {\ttfamily n}}}\label{README.md_autotoc_md28551}
Emulated flag {\ttfamily n} gives you {\itshape named capture only} mode, which turns unnamed groups {\ttfamily (…)} into noncapturing groups. It\textquotesingle{}s always implicitly on, though it doesn\textquotesingle{}t extend into interpolated {\ttfamily Reg\+Exp} instances (to avoid changing their meaning).

Requiring the syntactically clumsy {\ttfamily (?\+:…)} where you could just use {\ttfamily (…)} hurts readability and encourages adding unneeded captures (which hurt efficiency and refactoring). Flag {\ttfamily n} fixes this, making your regexes more readable.
...
\DoxyCodeLine{//\ Use\ standard\ (?<name>…)\ to\ capture\ as\ \`{}name`}

\end{DoxyCode}


\begin{DoxyNote}{Note}
Flag {\ttfamily n} is based on .NET, C++, Oniguruma, PCRE, Perl, and XReg\+Exp. It\textquotesingle{}s not always specified by a flag, but where it can be (.NET, PCRE, Perl, XReg\+Exp) it\textquotesingle{}s always {\ttfamily n}. The option is variously called {\itshape explicit capture}, {\itshape no auto capture}, {\itshape don\textquotesingle{}t capture group}, or {\itshape nosubs}. In Regex+, flag {\ttfamily n} also prevents using numbered backreferences to refer to named groups, which follows the behavior of C++ and the default handling of Oniguruma and Ruby. Referring to named groups by number is a footgun, and the way that named groups are numbered is inconsistent across regex flavors.
\end{DoxyNote}
\hypertarget{README.md_autotoc_md28552}{}\doxysubsubsection{\texorpdfstring{🧩 Interpolation}{🧩 Interpolation}}\label{README.md_autotoc_md28552}
\hypertarget{README.md_autotoc_md28553}{}\doxysubsubsubsection{\texorpdfstring{Interpolating regexes}{Interpolating regexes}}\label{README.md_autotoc_md28553}
...
\end{DoxyItemize}\end{adjustwidth}
\hypertarget{README.md_autotoc_md28554}{}\doxysubsubsubsection{\texorpdfstring{Interpolating escaped strings}{Interpolating escaped strings}}\label{README.md_autotoc_md28554}
...
{\ttfamily regex} solves all of this via context awareness. So instead of remembering anything above, you should just switch to always safely escaping regex syntax via {\ttfamily regex}.\hypertarget{README.md_autotoc_md28555}{}\doxysubsubsubsection{\texorpdfstring{Interpolating partial patterns}{Interpolating partial patterns}}\label{README.md_autotoc_md28555}
...
If you want to understand the handling of interpolated patterns more deeply, let\textquotesingle{}s look at some edge cases…



{\bfseries{👉 {\bfseries{Show some edge cases}}}}\begin{adjustwidth}{1em}{0em}
...
Although {\ttfamily \mbox{[}\texorpdfstring{$^\wedge$}{\string^}…\mbox{]}} is a negated character class, {\ttfamily \texorpdfstring{$^\wedge$}{\string^}} {\itshape within} a class doesn\textquotesingle{}t need to be escaped, even with the strict escaping rules of flags {\ttfamily u} and {\ttfamily v}.
...
\DoxyCodeLine{regex`(?<\$\{'name'\}>…)\(\backslash\)k<\$\{'name'\}>`}
...
\begin{DoxyEnumerate}
\item This is an uncompleted {\ttfamily \textbackslash{}u} token (which is an error) followed by the tokens {\ttfamily 0}, {\ttfamily 0}, {\ttfamily 0}, {\ttfamily A}. That\textquotesingle{}s because the interpolation doesn\textquotesingle{}t happen within an enclosed {\ttfamily \textbackslash{}u\{…\}} context.
...
\hypertarget{README.md_autotoc_md28556}{}\doxysubsubsubsection{\texorpdfstring{Interpolation principles}{Interpolation principles}}\label{README.md_autotoc_md28556}
...
\hypertarget{README.md_autotoc_md28557}{}\doxysubsubsubsection{\texorpdfstring{Interpolation contexts}{Interpolation contexts}}\label{README.md_autotoc_md28557}
\tabulinesep=1mm
...
\cellcolor{\tableheadbgcolor}\textbf{ Context }&\cellcolor{\tableheadbgcolor}\textbf{ Example }&\cellcolor{\tableheadbgcolor}\textbf{ String / coerced }&\cellcolor{\tableheadbgcolor}\textbf{ Pattern }&\cellcolor{\tableheadbgcolor}\textbf{ Reg\+Exp  }\\\cline{1-5}
\endhead
Default &{\ttfamily regex`\$\{'\texorpdfstring{$^\wedge$}{\string^}.+\textquotesingle{}\}{\ttfamily \texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Atomized \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Escaped\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Atomized\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Atomized \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Backrefs adjusted \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Flags localized\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}/tr\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}tr valign="{}top"{}\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}Character class\+: \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\mbox{[}…\mbox{]}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\mbox{[}\texorpdfstring{$^\wedge$}{\string^}…\mbox{]}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\mbox{[}\mbox{[}…\mbox{]}\mbox{]}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, etc.\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}regex}\mbox{[}\$\{\textquotesingle{}a-\/z\textquotesingle{}\}\mbox{]}{\ttfamily \texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Atomized \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Escaped\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Atomized\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}i\texorpdfstring{$>$}{>}Error\texorpdfstring{$<$}{<}/i\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}/tr\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}tr valign="{}top"{}\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}Interval quantifier\+: \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\{…\}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}regex}.\{1,\$\{5\}\}{\ttfamily \texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td rowspan="{}3"{}\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed \texorpdfstring{$<$}{<}br\texorpdfstring{$>$}{>} •\&nbsp;Escaped\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td rowspan="{}3"{}\texorpdfstring{$>$}{>}•\&nbsp;Sandboxed\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td rowspan="{}3"{}\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}i\texorpdfstring{$>$}{>}Error\texorpdfstring{$<$}{<}/i\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}/tr\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}tr valign="{}top"{}\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}Enclosed token\+: \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\textbackslash{}p\{…\}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\textbackslash{}P\{…\}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\textbackslash{}u\{…\}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\mbox{[}\textbackslash{}q\{…\}\mbox{]}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}regex}\textbackslash{}u\{\$\{\textquotesingle{}A0\textquotesingle{}\}\}{\ttfamily \texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}/tr\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}tr valign="{}top"{}\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}Group name\+: \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}(?\texorpdfstring{$<$}{<}…\texorpdfstring{$>$}{>})\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\textbackslash{}k\texorpdfstring{$<$}{<}…\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}, \texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}\textbackslash{}g\texorpdfstring{$<$}{<}…\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/code\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}/td\texorpdfstring{$>$}{>} \texorpdfstring{$<$}{<}td\texorpdfstring{$>$}{>}\texorpdfstring{$<$}{<}code\texorpdfstring{$>$}{>}regex}…\textbackslash{}k\texorpdfstring{$<$}{<}\$\{\textquotesingle{}a\textquotesingle{}\}\texorpdfstring{$>$}{>}\`{}}  \\\cline{1-5}
\end{longtabu}
...
\item Numbers interpolated into an enclosed {\ttfamily \textbackslash{}u\{…\}} context are converted to hexadecimal.
...
\end{DoxyItemize}\end{adjustwidth}
\hypertarget{README.md_autotoc_md28558}{}\doxysubsubsection{\texorpdfstring{🔩 Options}{🔩 Options}}\label{README.md_autotoc_md28558}
Typically, {\ttfamily regex} is used as follows\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{regex`…`\ //\ Without\ flags}
\DoxyCodeLine{regex('gi')`…`\ //\ With\ flags}
...
{\bfseries{👉 {\bfseries{Show details for each option}}}}\begin{adjustwidth}{1em}{0em}


{\bfseries{{\ttfamily flags}}} — For providing flags when using an options object.

{\bfseries{{\ttfamily subclass}}} — When {\ttfamily true}, the resulting regex is constructed using a {\ttfamily Reg\+Exp} subclass that avoids edge case issues with numbered backreferences. Without subclassing, submatches referenced {\itshape by number} from outside of the regex (e.\+g. in replacement strings) might reference the wrong values, because {\ttfamily regex}\textquotesingle{}s emulation of extended syntax (including atomic groups and subroutines) can add unnamed captures to generated regex source that might affect group numbering.

Context\+: {\ttfamily regex}\textquotesingle{}s implicit flag {\ttfamily n} ({\itshape named capture only} mode) means that all captures have names, so normally there\textquotesingle{}s no need to reference submatches by number. In fact, flag {\ttfamily n} {\itshape prevents} you from doing so within the regex. And even in edge cases (such as when interpolating {\ttfamily Reg\+Exp} instances with numbered backreferences, or when flag {\ttfamily n} is explicitly disabled), any numbered backreferences within the regex are automatically adjusted to work correctly. However, issues can arise if you reference submatches by number (instead of their group names) from outside of the regex. Setting {\ttfamily subclass\+: true} resolves this, since the subclass knows about added "{}emulation groups"{} and automatically adjusts match results in all contexts.

\begin{quote}
This option isn\textquotesingle{}t enabled by default because it would prevent Regex+\textquotesingle{}s Babel plugin from emitting regex literals. It also has a small performance cost, and is rarely needed. The primary use case is tools that use {\ttfamily regex} internally with flag {\ttfamily n} disabled. \end{quote}


{\bfseries{{\ttfamily plugins}}} — An array of functions. Plugins are called in order, after applying emulated flags and interpolation, but before the built-\/in plugins for extended syntax. This means that plugins can output extended syntax like atomic groups and subroutines. Plugins are expected to return an updated pattern string, and are called with two arguments\+:
...
{\bfseries{{\ttfamily disable}}} — A set of options that can be individually disabled by setting their values to {\ttfamily true}.


\begin{DoxyItemize}
\item {\bfseries{{\ttfamily x}}} — Disables implicit, emulated flag {\ttfamily x}.
\item {\bfseries{{\ttfamily n}}} — Disables implicit, emulated flag {\ttfamily n}. Note that, although it\textquotesingle{}s safe to use unnamed captures and numbered backreferences within a regex when flag {\ttfamily n} is disabled, referencing submatches by number from {\itshape outside} a regex (e.\+g. in replacement strings) can result in incorrect values because extended syntax (atomic groups and subroutines) might add "{}emulation groups"{} to generated regex source. It\textquotesingle{}s therefore recommended to enable the {\ttfamily subclass} option when disabling {\ttfamily n}.
\item {\bfseries{{\ttfamily v}}} — Disables implicit flag {\ttfamily v} even when it\textquotesingle{}s supported natively, resulting in flag {\ttfamily u} being added instead (in combination with the {\ttfamily unicode\+Sets\+Plugin}).
\item {\bfseries{{\ttfamily atomic}}} — Disables atomic groups and possessive quantifiers, resulting in a syntax error if they\textquotesingle{}re used.
\item {\bfseries{{\ttfamily subroutines}}} — Disables subroutines and subroutine definition groups, resulting in a syntax error if they\textquotesingle{}re used.
\end{DoxyItemize}

{\bfseries{{\ttfamily force}}} — Options that, if set to {\ttfamily true}, override default settings (as well as options set on the {\ttfamily disable} object).


\begin{DoxyItemize}
\item {\bfseries{{\ttfamily v}}} — Forces the use of flag {\ttfamily v} even when it\textquotesingle{}s not supported natively (resulting in an error). 
\end{DoxyItemize}\end{adjustwidth}
\hypertarget{README.md_autotoc_md28559}{}\doxysubsubsubsection{\texorpdfstring{Returning a string}{Returning a string}}\label{README.md_autotoc_md28559}
Function {\ttfamily rewrite} returns an object with properties {\ttfamily expression} and {\ttfamily flags} as strings, rather than returning a {\ttfamily Reg\+Exp} instance. This can be useful when you want to apply postprocessing to the output.


\begin{DoxyCode}{0}
\DoxyCodeLine{import\ \{rewrite\}\ from\ 'regex';}
\DoxyCodeLine{rewrite('\string^\ (ab\ |\ cd)',\ \{flags:\ 'm'\});}
\DoxyCodeLine{//\ →\ \{expression:\ '\string^(?:ab|cd)',\ flags:\ 'mv'\}}

\end{DoxyCode}


{\ttfamily rewrite} shares all of {\ttfamily regex}\textquotesingle{}s options (described above) except {\ttfamily subclass}. Providing the resulting {\ttfamily expression} and {\ttfamily flags} to the {\ttfamily Reg\+Exp} constructor produces the same result as using the {\ttfamily regex} tag.
...
\hypertarget{README.md_autotoc_md28560}{}\doxysubsubsection{\texorpdfstring{⚡ Performance}{⚡ Performance}}\label{README.md_autotoc_md28560}
...
\end{DoxyItemize}\hypertarget{README.md_autotoc_md28562}{}\doxysubsubsection{\texorpdfstring{🙋 FAQ}{🙋 FAQ}}\label{README.md_autotoc_md28562}
...
{\bfseries{{\bfseries{Why are flags added via {\ttfamily regex(\textquotesingle{}g\textquotesingle{}){\ttfamily …}} rather than {\ttfamily regex{\ttfamily /…/g}}?}}}}\begin{adjustwidth}{1em}{0em}
...
\item It would most likely be incompatible if a standardized regex template tag was added to the Java\+Script language in the future. To date, TC39 discussions about a standardized tag for regexes have not favored the {\ttfamily \`{}/…/g\`{}} format. 
\end{DoxyItemize}\end{adjustwidth}
\hypertarget{README.md_autotoc_md28563}{}\doxysubsubsection{\texorpdfstring{🏷️ About}{🏷️ About}}\label{README.md_autotoc_md28563}
