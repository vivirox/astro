...
export function Alert({
  ...
  // Icon based on variant if not provided
  const defaultIcon = {
    info: (
      <svg
  ...
  return (
    <div className={classes} style={style} role="alert">
      ...
      {dismissible && onDismiss && (
        <button
          type="button"
          className="absolute top-1 right-1 inline-flex text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 focus:outline-none p-1.5 rounded-md"
          onClick={onDismiss}
