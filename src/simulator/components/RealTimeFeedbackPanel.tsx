...
export default function RealTimeFeedbackPanel({
  ...
  useEffect(() => {
    ...
    const empathyScore = (emotionInsights.valence * 0.7) + ((1 - emotionInsights.dominance) * 0.3);
      ...
      case 'active_listening':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15.536a5 5 0 001.414 1.414m2.828-4.242a1 1 0 011.414 0 1 1 0 010 1.414" />
          </svg>
        );
      default:
        return (
  ...
  return (
    <div className={`real-time-feedback-panel rounded-lg border border-gray-200 shadow-sm bg-white overflow-hidden ${className}`}>
      {/* Header */}
      ...
      <div className="p-3">
        {/* Empathy Meter */}
        <div className="mb-4">
          <h4 className="text-xs font-medium text-gray-500 mb-2">THERAPEUTIC PRESENCE</h4>
          <div className="flex justify-center">
            <EmpathyMeter
              value={empathyHistory.length > 0 ? empathyHistory[empathyHistory.length - 1] : 0.5}
        ...
        {/* Detected Techniques */}
        <div className="mb-4">
          <h4 className="text-xs font-medium text-gray-500 mb-2">DETECTED THERAPEUTIC TECHNIQUES</h4>
          {detectedTechniques.length > 0 ? (
            ...
            <p className="text-sm text-gray-500 italic">No techniques detected yet</p>
          )}
        </div>
        ...
        {highPriorityFeedback && (
          <div className="mb-4">
            <h4 className="text-xs font-medium text-gray-500 mb-2">PRIORITY FEEDBACK</h4>
            <div className={`rounded-lg border p-3 ${getPriorityColorClass(highPriorityFeedback.priority)}`}>
              <div className="flex gap-2">
                {getFeedbackIcon(highPriorityFeedback.type)}
                <div>
                  <p className="text-sm font-medium text-gray-800 mb-1">{highPriorityFeedback.suggestion}</p>
                  <p className="text-xs text-gray-600">{highPriorityFeedback.rationale}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div>
          <h4 className="text-xs font-medium text-gray-500 mb-2">FEEDBACK HISTORY</h4>
          {feedback.length > 0 ? (
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {feedback.slice().reverse().map((item, index) => (
                <div
                  ...
                  onClick={() => setExpandedFeedback(expandedFeedback === `${item.timestamp}-${index}` ? null : `${item.timestamp}-${index}`)}
                >
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 text-gray-400 transition-transform ${expandedFeedback === `${item.timestamp}-${index}` ? 'transform rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {expandedFeedback === `${item.timestamp}-${index}` && (
                    <div className="mt-2 ml-7 text-xs text-gray-600">
                      ...
                      <p className="text-gray-500">
                        ...
                        {item.context && ` ${formatTechnique(item.context)} context`}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <p className="text-sm text-gray-500 italic">No feedback recorded yet</p>
          )}
        </div>
      </div>
