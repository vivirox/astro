...
const initialMetrics: AnonymizedMetrics = {
  techniquesUsed: Object.values(TherapeuticTechnique).reduce(
    (acc, technique) => {
      acc[technique] = 0
      return acc
    },
  ...
  domainsExplored: Object.values(TherapeuticDomain).reduce(
    (acc, domain) => {
      acc[domain] = 0
      return acc
    },
  ...
  feedbackReceived: Object.values(FeedbackType).reduce(
    (acc, type) => {
      acc[type] = 0
      return acc
    },
  ...
  skillProgress: Object.values(TherapeuticTechnique).reduce(
    (acc, technique) => {
      acc[technique] = 0
      return acc
    },
      ...
      setMetrics((current) => {
        ...
        switch (action.type) {
          case 'startSession':
            ...
            if (
              Object.values(TherapeuticDomain).includes(
                action.domain as TherapeuticDomain,
              )
            ) {
