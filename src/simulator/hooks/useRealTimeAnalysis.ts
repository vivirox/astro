...
import { FeedbackType, RealTimeFeedback, Scenario, TherapeuticTechnique } from '../types';
...
export function useRealTimeAnalysis(scenario: Scenario | null) {
  ...
  const [feedbackService] = useState<FeedbackService>(() => new FeedbackService());
  ...
  const startAudioAnalysis = useCallback((stream: MediaStream) => {
    ...
    const analysisInterval = setInterval(() => {
      ...
      feedbackService.processFeedback(mockAudioChunk, 0.5)
        .then((result) => {
          if (result) {
  ...
  const simulateEmotionInsights = useCallback(() => {
    ...
    setEmotionInsights((prev) => {
      return {
        energy: Math.max(0, Math.min(1, prev.energy + (Math.random() * 0.2 - 0.1))),
        valence: Math.max(0, Math.min(1, prev.valence + (Math.random() * 0.2 - 0.1))),
        dominance: Math.max(0, Math.min(1, prev.dominance + (Math.random() * 0.2 - 0.1)))
      };
    });
  }, []);
