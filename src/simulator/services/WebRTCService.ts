...
export class WebRTCService implements WebRTCServiceInterface {
   ...
   */
  private createSimulatedAnswer(offer: RTCSessionDescriptionInit): RTCSessionDescriptionInit {
      ...
      simulatedCandidates.forEach(candidate => {
        if (this.peerConnection) {
          this.peerConnection.addIceCandidate(candidate)
            .catch(err => console.error('Error adding simulated ICE candidate:', err));
        }
      });
    }, 800);
  }
  ...
  private createSimulatedIceCandidates(): RTCIceCandidate[] {
    ...
    return [
      ...
      new RTCIceCandidate({
        candidate: 'candidate:2 1 UDP 1845501695 203.0.113.100 50001 typ srflx raddr 192.168.1.100 rport 50000',
        sdpMid: '0',
        sdpMLineIndex: 0
      })
    ];
