import { FeedbackModelConfig, FeedbackServiceInterface, FeedbackType, RealTimeFeedback, SimulationScenario, Scenario, TherapeuticTechnique } from '../types';
...
export class FeedbackService implements FeedbackServiceInterface {
  ...
  private emotionState: {
    ...
    valence: number;
    dominance: number;
    trends: Array<{timestamp: number, energy: number, valence: number, dominance: number}>
  } = {
    energy: 0.5,
    valence: 0.5,
    dominance: 0.5,
   ...
   */
  private analyzeAudioCharacteristics(audioData: Float32Array, sampleRate: number): void {
   ...
   */
  private detectSpeechPatterns(audioData: Float32Array, sampleRate: number): void {
    // Calculate RMS
    let sumSquares = 0;
    for (let i = 0; i < audioData.length; i++) {
      sumSquares += audioData[i] * audioData[i];
  ...
  async processFeedback(
    ...
    const currentApproach = this.analyzeTherapeuticApproach(currentValence, currentEnergy);
    ...
    if (emotionChange && Math.random() < 0.7) {
      ...
      return {
        ...
        timestamp: Date.now(),
        suggestion: "Consider slowing your speaking pace slightly to give the client more processing time.",
        rationale: "A moderate speaking rate helps clients absorb complex emotional content, particularly when discussing sensitive topics.",
        priority: 'medium',
      ...
      return {
        ...
        timestamp: Date.now(),
        suggestion: "Try incorporating more brief pauses to allow the client space to process.",
        rationale: "Strategic pauses demonstrate attentive listening and create space for deeper reflection.",
        priority: 'low',
        context: this.currentScenario.domain
      };
    } else if (currentDominance > 0.7 && this.currentScenario.domain === 'trauma') {
      // Feedback on counselor dominance in trauma scenarios
      return {
        type: 'therapeutic_alliance',
        timestamp: Date.now(),
        suggestion: "Consider a more collaborative approach that emphasizes client autonomy and choice.",
        rationale: "In trauma work, maintaining client agency is particularly important for building safety and trust.",
        priority: 'high',
        context: 'trauma'
      };
    }
  ...
  private detectEmotionalChange(): 'positive' | 'negative' | 'neutral' | null {
    ...
    const significantChange = Math.abs(valenceChange) > 0.1 || Math.abs(energyChange) > 0.15;
  ...
  private generateEmotionFeedback(
    ...
    if (emotionChange === 'positive') {
      return {
        ...
        timestamp: Date.now(),
        suggestion: "The client's emotional state appears to be shifting positively. Consider acknowledging this change.",
        rationale: "Recognizing positive emotional shifts reinforces progress and helps build therapeutic momentum.",
        priority: 'medium',
        context: this.currentScenario?.domain
      };
      ...
      if (currentApproach === TherapeuticTechnique.COGNITIVE_RESTRUCTURING) {
        return {
          ...
          timestamp: Date.now(),
          suggestion: "The client may need validation before cognitive restructuring as their emotional state intensifies.",
          rationale: "Validation creates safety during heightened emotions, making clients more receptive to cognitive work later.",
          priority: 'high',
          context: this.currentScenario?.domain
        };
      }
      ...
      return {
        type: 'therapeutic_alliance',
        timestamp: Date.now(),
        suggestion: "The client's emotional intensity is increasing. Consider validating their experience before proceeding.",
        rationale: "Validation during emotional intensity strengthens the therapeutic alliance and models emotional acceptance.",
        priority: 'high',
        context: this.currentScenario?.domain
      };
    } else {
      return {
        ...
        timestamp: Date.now(),
        suggestion: "Consider using a reflective statement to clarify the client's current emotional experience.",
        rationale: "Reflection helps clients articulate emotional experiences that may be difficult to express directly.",
        priority: 'low',
        context: this.currentScenario?.domain
      };
    }
