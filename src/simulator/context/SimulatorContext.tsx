...
import { FeedbackModelConfig, RealTimeFeedback, SimulationScenario, SimulatorContextType, WebRTCConnectionConfig } from '../types';
...
export const SimulatorProvider: React.FC<SimulatorProviderProps> = ({
  ...
  const [feedbackService] = useState<FeedbackService>(() => new FeedbackService(feedbackModelConfig));
  ...
  const handleRemoteStream = useCallback((stream: MediaStream) => {
    ...
    const interval = setInterval(() => {
      ...
      if (isConnected && currentScenario && !isAudioProcessing) {
        ...
        feedbackService.processFeedback(dummyAudioChunk, 1.0)
          .then(feedback => {
            if (feedback) {
  ...
  }, [isConnected, currentScenario, isAudioProcessing, feedbackService, handleNewFeedback]);
  ...
  const startSimulation = useCallback(async (scenarioId: string) => {
    try {
      ...
      await webRTCService.createLocalStream({ echoCancellation: true, noiseSuppression: true }, false);
